...
Más adelante, a finales de la década, se presentó el modelo \textbf{GM}\sidenote{
	Desarrollado dentro de la \textit{General Motors}, de ahí su nombre.
} (\cite{Chandler1958}), el cual sirvió como base para el desarrollo de modelos posteriores. Éste se caracteriza por el uso del concepto \textbf{estímulo $\rightarrow$ respuesta}, donde la respuesta del vehículo (el cambio en la tasa de aceleración) es debida a la activación de un estímulo (la variación en la distancia con el vehículo delantero) tras pasar un tiempo de retardo $\tau$\sidenote{
