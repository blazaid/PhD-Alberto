...
En un microsimulador de tráfico basado en \gls{mas}, cada uno de los agentes trabaja de manera independiente sobre el mismo problema (el tráfico), y esto nos ofrece un entorno ideal para modelar y estudiar el comportamiento de cada uno de ellos independientemente.

La mayoría de los modelos de comportamiento existentes (\TODO{Me parece que a partir del 2000 o así ha habido un despunte. Comprobar}) basan su funcionamiento en técnicas pertenecientes al \glsentrylong{hc} (recordemos: aquellas basadas en fórmulas matemáticas y reglas de la lógica convencional) con parámetros que se ajustan a partir de la observación de datos reales. Sin embargo, en los últimos años ha empezado a crecer el interés por técnicas de \gls{ci} en el campo y por tanto existen ciertos estudios de técnicas más acorde con este punto de vista.

En este capítulo se explorará qué modelos de interés se han desarrollado hasta el momento bajo el praaguas de la \gls{ci}.

\section{Comportamiento en la conducción}

El término \textit{comportar} se define como \textit{actuar de una manera determinada}. El comportamiento al volante se refiere a la manera que tiene un conductor de comportarse durante la tarea de la conducción. Por tanto, para determinar el comportamiento durante la conducción de un individuo es necesario identificar qué tareas son clave durante la ejecución de la misma.
...
Éstas tareas no son úncamente aquellas relacionadas con la interacción con el vehículo. Incluyen otras muchas tareas (e.g. identificar la situación en la que se encuentra inmerso, las señales, los peatones, \ldots), cada una de diferente nivel cognitivo.

En \cite{michon1985critical} el autor divide en tres los niveles de abstracción de las tareas involucradas durante la conducción: el de \textbf{control}, que se ocupa de las tareas de más bajo nivel como aceleración, freno, cambios de marcha destinadas a mantener la conducción, el de \textbf{maniobra} o táctico, donde sus tareas son las encargadas de mantener la interacción con el entorno como los cambios de carril o el control de las señales y demás estímulos externos, y el \textbf{estratégico}, que engloba las tareas de más alto nivel como el razonamiento y la planificación de rutas (ver figura~\ref{fig:three-levels-of-human-driving}).

Según esta jerarquía, el aspecto del comportamiento sobre el que trabajamos en esta tesis es el del nivel de \textbf{maniobra}. Ésta se puede concebir como la encargada de planificar acciones a corto plazo para conseguir objetivos a corto plazo. Planificaciones a más alto nivel como el de la planificación de una ruta o de más bajo nivel como decidir qué presión aplicar en el freno quedan fuera del estudio.
...
\section{Modelos a reproducir}

En el nivel táctico del comportamiento, las tareas que se realizan durante la conducción son las relacionadas a circular con el vehículo dentro del flujo de tráfico, sin bajar demasiado de detalle. En la literatura, estas tareas se centran en dos clases de tarea diferentes (figura \ref{fig:behavior-model-classification}): la de \textbf{aceleración}, encargadas de modelar el comportamiento de un conductor en el carril en el que se encuentra y las de \textbf{cambio de carril}, encargadas de decidir y ejecutar los cambios de carril.

Estas dos clases se han tratado tradicionalmente como modelos independientes, siendo los modelos de aceleración mucho más estudiados en la literatura respecto al los de cambio de carril\sidenote{Este hecho es motivado por la dificultad en la captura de datos en el cambio de carril y por tanto en su escasez.}.
...
\section{La \glsentryshort{ci} aplicada a modelos de conducción}

Nosotros nos centramos en \acrshort{ai}, seguramente haya que recortar un poco lo de arriba para nombrarlo por encima. Quizá apuntando a los distintos tipos y ya (modelso no basados en \gls{ci} quiero decir). Según~\cite{Aghabayk2015}, los modelos basados en \acrlongsp{ai} funcionan en base a dos técnicas principalmente, las \acrlongplsp{ann} y la \acrlongsp{fl}.
