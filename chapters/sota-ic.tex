...
Se puede establecer el año $1994$ como en el que nace formalmente la \glsentrylongplsp{ci} nace como área, coincidiendo con el cambio de nombre del \textit{IEEE Neural Networks Council} a \textit{IEEE Computational Intelligence Society}\sidenote{
...
Este primer modelo de neurona proponía una función escalón para determinar si la neurona se activaba o no (analogía al funcionamiento de la neurona artificial). Posteriormente aparecieron nuevos modelos de neuronas con diferentes funciones de activación. De éstas, las más comunes son las de tipo sigmoide\sidenote{
...
En esta sección hablaremos de los \glsentrylongsp{ga} como técnica base de la \gls{cev}. Sin embargo, existen familias de algoritmos consideradas por muchos autores como entidades diferentes a los \glspl{ga}:
...
\marginnote{
	...
	\begin{itemize}
		\item El principio de \textbf{selección natural}. Introducido en la teoría de la evolución, indica que los rasgos de un individuo e facilitan (o dificultan) sus posibilidades de reproducción en un entorno, y por tanto de transmisión de sus rasgos.
		\item La \textbf{teoría genética}, por Gregor Mendel. Concretamente los conceptos de \textit{gen} (unidad de información genética), alelo (variación de valor de un gen que da lugar a variaciones en el fenotipo), genotipo (conjunto de genes que caracterizan a un individuo), fenotipo (las características físicas del individuo, determinadas por el genotipo y el entorno) y la herencia genética.
		\item La \textbf{mutación} como fuente de variación dentro de una población.
		\item El principio de \textbf{diversidad alélica} de la genética de poblaciones.
	\end{itemize}
}

La estructura de un \gls{ga} se asemeja al proceso de evolución de poblaciones finitas de individuos. Un ejemplo de esta estructura lo podemos ver en la figura~\ref{fig:genetic-algorithm-schema} Los principales pasos dentro del algoritmo son los siguientes:

\begin{figure}[t]
	\missingfigure[figheight=3.5cm]{Esquema general de funcionamiento de un algoritmo genético}
	\caption[Esquema general del funcionamiento de un \glsentrylongsp{ga}]{Esquema general del funcionamiento de un \glsentrylong{ga}. Se parte de una población de individuos y en iteraciones de selección-recombinación-mutación-reemplazo se va mejorando la población hasta llegar a una solución que, si bien no tiene por qué ser necesariamente la mejor, es lo suficientemente buena como para ser válida.}
...
\newthought{Según converge el algoritmo}, la biodiversidad disminuye. Esto es debido a que cuando según vamos acercando nuestra población a una región del espacio, todos los valores que se toman tienden a ser cercanos a esta posición. Esta característica se denomina \textit{explotación}, y es el caso contrario a la \textit{exploración}, es decir, cuando los individuos están distribuidos por todo el espacio de búsqueda buscando posibles soluciones.
...
\begin{figure}
	...
	\caption[Técnicas de selección más comunes en \glsentrylongplsp{ga}]{Tres ilustraciones de las técnicas de selección más comunes en \glsentrylong{ga}. De izquierda a derecha (a) selección proporcional al fitness, (b) selección proporcional a la posición y (c) selección por torneo.}
...
\begin{figure}[t]
	\missingfigure[figheight=4cm]{Un ejemplo de un problema y una codificación de ese problema en binario y en real.}
...
\begin{figure}[t]
	...
	\caption[Diferencias entre representación de un genotipo como árbol tradicional y como árbol generado a partir de una gramática libre de contexto]{Diferencias entre un árbol que codifica un programa en \glsentryshort{gp} y uno que codifica una palabra de una gramática libre de contexto en \glsentrylong{gggp}.}
