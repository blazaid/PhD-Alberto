...
\marginnote{
	El entorno de simulación \gls{torcs}, usado en multitud de concursos e investigaciones, se trata de un juego. Los juegos son un \textit{sandbox} perfecto ya que presentan una abstracción de complejidad acotada sobre el dominio que trabajar.
...
\begin{figure*}[t]
	...
	ROOT/.style = {fill=blue!30, inner sep=2mm, font=\bfseries},
	...
	LD1/.style = {level distance=5ex},
	LD2/.style = {level distance=10ex},
	LD3/.style = {level distance=15ex},
	LD4/.style = {level distance=20ex},
	level 1/.style = {sibling distance=40mm}
	]
	\node[ROOT] {Aspectos medibles}
	[edge from parent fork down]
	child{node[L2] {Complejidad}
		child[L3,LD1] {node[L3] {micro}}
		child[L3,LD2] {node[L3] {macro}}
		child[L3,LD3] {node[L3] {meso}}
		child[L3,LD4] {node[L3] {\ldots}}
	}
	child{node[L2] {¿Qué medir?}
		child[L3,LD1] {node[L3] {vehículos}}
		child[L3,LD2] {node[L3] {flujo en vía}}
		child[L3,LD3] {node[L3] {componentes}}
		child[L3,LD4] {node[L3] {\ldots}}
	}
	child {node[L2] {¿Cómo medir?}
		child[L3,LD1] {node[L3] {\glsentrylongplsp{mas}}}
		child[L3,LD2] {node[L3] {\glsentrylongplsp{ca}}}
		child[L3,LD3] {node[L3] {Ecuaciones diferenciales}}
		child[L3,LD4] {node[L3] {\ldots}}
	};
	\end{tikzpicture}
	\caption{Los aspectos medibles del problema del táfico son muy diversos, y dependen del nivel de granularidad (i.e. complejidad) al que se quiere llegar, de qué queremos medir y de cómo lo queremos hacer.}
...
\begin{itemize}
	\item \textbf{Microsimulación} o simulación de tipo \textbf{micro}. Su objetivo es estudiar, desde un punto de vista de granularidad fina (e.g. vehículos o peatones), las micropropiedades del flujo de tráfico como, por ejemplo, los cambios de carril, las aproximaciones a vehículos delanteros o los adelantamientos, para evaluar su comportamiento. Sus dos principales ventajas son la posibilidad de estudiar el tráfico como un todo a partir de sus elementos más simples (ofreciendo una representación más fiel de éste) y la posibilidad de estudiar cada elemento por separado. Sin embargo, su principal desventaja es que cada elemento de la simulación requiere de cómputo independiente y por tanto simulaciones con alto contenido de elementos pueden llegar a ser inviables\sidenote{
		Existen técnicas de computación distribuida que superan ampliamente los límites impuestos por la computación en un único nodo. Un ejemplo relativamente reciente es el simulador de IBM \textit{Megaffic}. Éste implementa un modelo de granularidad micro donde cada elemento es un agente independiente (i.e. \glsentrylongplsp{mas}) usando para ello entornos con cientos de núcleos de proceso que proveen de capacidad suficiente para modelar ciudades enteras como Tokio (ver~\cite{Osogami2012} y~\cite{Suzumura2012}).
	}.
...
\begin{figure*}[t]
	...
	\caption{Ejemplo de un modelo lineal en un espacio continuo. La posición del vehículo es un valor $x \in \mathbb{R}$. Este ejemplo muestra un modelo de \textit{car-following} donde el comportamiento de la aceleración del vehículo es determinado por la distancia al coche siguiente. Fuente:~\cite{Tordeux2011}.}
...
Los simuladores que se basan en un modelo de granularidad micro están en su mayoría implementados en dos tipos de paradigma: \glsentrylongplsp{ca} y \glsentrylongplsp{mas}.
...
\subsection{Microsimulación basada en \glsentrylongplsp{ca}}

\begin{figure}[t]
	\centering
	\includegraphics{images/nagel-schreck}
	\caption{Aparición de retenciones en una autopista de $250$ celdas usando el modelo Nagel-Scherckenberg. La densidad de ocupación es de $50$ coches en la vía, la velocidad máxima es de $5 c/\Delta t$ y la probabilidad de frenada es de $p = 0.5$. Se puede observar en la figura cómo se desplazan las olas del atasco a lo largo de las $100$ iteraciones. Fuente: simulador \texttt{nagel-scherckenberg-demo} (Ver capítulo \nameref{ch:developed-software}).}
	\label{fig:nagel-schreck}
\end{figure}

Un \ac{ca} es una colección ordenada de celdas (\textit{células}) ordenadas en un espacio $n$-dimensional que parcelan el universo de estudio. Cada una de ellas se encuentra en un estado (e.g. contiene un valor numérico), y el estado de toda la malla se actualiza de manera síncrona\sidenote{
...
\marginnote{
	...
	\begin{itemize}
		...
		\item $\Delta t$ queda establecido en \unit{1}{s.}, considerado el tiempo medio de reacción de un conductor ante una eventualidad. Esto hace, por ejemplo, que una velocidad de $6 c/\Delta t$ sea $45 m/s$ ($162 km/h$).
...
\begin{figure}[t]
	...
	LD3/.style = {level distance=15ex},
	LD4/.style = {level distance=20ex},
	LD5/.style = {level distance=25ex},
	level 1/.style = {sibling distance=45mm}
	]
	\node[ROOT] {Características}
	[edge from parent fork down]
	child{node[L2] {Obligatorias}
		child[L3,LD1] {node[L3] {Tipo de simulador}}
		child[L3,LD2] {node[L3] {Modelo de simulación}}
		child[L3,LD3] {node[L3] {Entorno de simulación}}
		child[L3,LD4] {node[L3] {Extensibilidad}}
		child[L3,LD5] {node[L3] {Sistema operativo (GNU/Linux)}}
	}
	child {node[L2] {Opcionales}
		child[L3,LD1] {node[L3] {Activo}}
		child[L3,LD2] {node[L3] {Lenguaje de programación}}
		child[L3,LD3] {node[L3] {Licencia}}
		child[L3,LD4] {node[L3] {Sistema operativo (OS-X)}}
	};
	\end{tikzpicture}
	\caption{Características obligatorias y deseables del simulador donde implementar nuestros modelos personalizados de conductor.}
...
\begin{table*}[t]
	\caption[Tabla comparativa de los simuladores seleccionados][1.5em]{Tabla comparativa donde se contrastan las caracterísitcas de los simuladores seleccionados. El simulador \gls{sumo} es el que más características cumple de las originalmente planteadas.}
...
\begin{figure}[t]
	...
	\begin{tikzpicture}[
		...
		blueb/.style={
			...
			text height=12pt,
			text depth=9pt},
		greenb/.style={blueb,fill=green},
	]
		\node[greenb,fill=orange,text width=7.5cm+44pt] (sumo-traffic-simulation-suite) {SUMO Traffic Simulation Suite};
		\node[blueb,below=of sumo-traffic-simulation-suite,text width=7.5cm+26pt] (traffic-control-interface) {TraCI Traffic Control Interface};
		\node[blueb,below=of traffic-control-interface,text width=7.5cm+26pt] (web-service) {Servicios Web};
		\node[blueb, fill=orange, below=of web-service] (python-client) {Cliente Python};
		\node[blueb, fill=orange, left=of python-client]
			(java-client) {Cliente Java};
		\node[blueb, fill=orange, right=of python-client]
			(other-clients) {Otros clientes};
	\end{tikzpicture}
