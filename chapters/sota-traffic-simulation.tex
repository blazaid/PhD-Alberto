...
El caso de los simuladores de tráfico es el mismo\sidenote{No entraremos a hablar sobre las técnicas de computación distribuida y de cómo éstas superan los límites impuestos por la computación en un único nodo, pero merece la pena mencionar que existen también avances en este campo, como por ejemplo el simulador de IBM \textit{Megaffic}, donde se implementa un modelo de granularidad micro de tipo sistema multiagente, haciendo uso de computación paralela y distribuida, usando para ello entornos con cientos de núcleos, dando la capacidad para simular ciudades enteras como Tokio (ver~\cite{Osogami2012} y~\cite{Suzumura2012}).}. En éstos existe un amplio intervalo de granularidades, desde por ejemplo el flujo de entrada en una autovía hasta el consumo de carburante de un vehículo en ciudad. Lo más común es clasificar los simuladores dentro de dos grandes grupos, los cualesse ilustran en la Figura~\ref{fig:granularities-in-traffic-simulation}.
