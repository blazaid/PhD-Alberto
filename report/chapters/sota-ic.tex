...
Hablar del modelo de neurona artificial, de feed-forward vs recurrentes y del efecto de las diferentes funciones de activación.

En esta tesis el trabajo se centra en dos topologías concretas dentro de las redes de tipo \textit{feed-forward}, el \acrlong{mlp} y la \acrlong{cnn}, que son con las que se cerrará la sección. ReLU viene al rescate (a(x) = max(0, x)), no tiene derivada, pero se puede decir que en 0 es 0 o 1, nos da igual.
...
El valor de salida de una neurona queda determinado por la aplicación de una función sobre la entrada neta a ésta. Esta función dictamina el grado de activación de la neurona y para un correcto funcionamiento de la red en términos generales debe ser no lineal\sidenote{
	Supongamos que tenemos una estructura lineal como la siguiente:
	
	\includegraphics{mlp-linear}
	
	Si las funciones de activación de las neuronas fuesen lineales (por ejemplo, $f(x) = x$), tendríamos el siguiente desarrollo:
	
	\begin{align*}
	y^1 = f(W_1 * \mathbf{x} + \mathbf{b_1}) \\
	y = y^2 = f(W_2 * \mathbf{y^1} + \mathbf{b_2}) \\
	\end{align*}
	...
	Sí podría tener sentido en la capa de salida en algunos problemas de regresión, por ejemplo, al intentar ajustar a un valor que crece o decrece con el tiempo y cuyos límites desconocemos, aunque en este tipo de problemas también podría servir una función de activación de tipo ReLU o Leaky ReLU.
}.

Las funciones de activación clásicas usadas en redes neuronales han sido la función sigmoidal y la tangente hiperbólica
...
\begin{figure}[t]
	...
	\caption{Lasfunciones de activación sigmoida y tangente hiperbólicas. Ambas se han usado como funciones de activación no lineales gracias a que sus derivadas son continuas a lo largo de todo el dominio y además son fácilmente computables.}
	\label{fig:dropout-example}
\end{figure}


Hablar de la sigmoidal y de la tanh, que es mejor en términos generales porque tienede a centrar los datos, lo cual es bueno para pasar los datos de una capa a otra.

Un problema de estas funciones de activación es que si el valor absoluto de x es muy grande, la derivada tiende a 0, por lo que los entrenamientos, que en su mayoría se basan en el descenso del gradiente se hacen muy lentos. Por eso se decía que los valores de inicio de los pesos fuesen valores muy pequeños diferentes de 0.

Para elegir, en general, si se está haciendo clasificación binaria o regresión, salida una sigmoidal y para el resto ReLU (es muy rápido) o tanh.

una de las desventajas de la ReLU es que la derivada es 0 en valores menores que 0, por lo que las neuronas ``mueren'' si llegan a este punto. Hay una Leaky ReLU, pero no se usa demasiado.

sigmoide: nunca a no se que se haga clasificación binaria, que entonces se usa en la salida. más que nada porque tanh es similar pero centra los datos.

ReLU default

$sig(x) = \frac{1}{1 + e^{-x}}$. Slope

$tanh(x) = \frac{e^x - e^{-x}}{e^x + e^x}$

$relu(x) = \max{0, x}$

$lrelu = \max{\epsilon x, x}$ Epsilon pequeñito, como 0.01. Se puede convertir como un parámetro más a optimizar.
