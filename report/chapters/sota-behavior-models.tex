...
Desde mediados de los años $1990$ empezó a crecer el interés por aplicar la \acrlongsp{ai} a los \gls{its}\index{ITS}. Hay dos razones por las que esto es así: la primera, los éxitos cosechados por la \acrlongsp{ci}, los cuales atrayeron a investigadores de multitud de áreas incluida ésta. La segunda, el rápido desarrollo de la tecnología\sidenote{
	La tecnología es cada vez más barata, más precisa y con más funcionalidades. En la última década hemos vivido una explosión de dispositivos de todo tipo: teléfonos y relojes con GPS, acelerómetros y giroscopio, ordenadores completamente funcionales del tamaño de una moneda, sensores RADAR y \acrshort{lidar} para uso amateur además de profesional y así un largo etcétera.
...
Las técnicas de \gls{ci} más utilizadas en el modelado de conductores son las \acrlongsp{ann}\index{red neuronal artificial} y la \acrlongsp{fl}\index{lógica difusa}. Es comprensible dado que las primeras son una de las técnicas principales en la rama del \acrlongsp{ml}\index{aprendizaje automático}, y la segunda por ser una manera sencilla y cercana a la manera de razonar del ser humano.
...
\newthought{Las \acrlongsp{ann}\index{red neuronal artificial}} fueron el punto de entrada de la \acrlongsp{ci} en la modelización de conductores. Los \acrlongsp{mlp}\index{perceptrón multicapa} y sus nuevas técnicas de entrenamiento se habían convertido en la nueva solución universal para todo aquel problema del que se dispusiese de datos.

Las \acrlongsp{ann}\index{red neuronal artificial} se han aplicado mucho sobre el campo de los \acrshort{its}\index{ITS} en general, no sólo en modelado de conductores sino en prácticamente todos los aspectos como la clasificación de conductores \cite{DiazAlvarez2014}, los sistemas de conducción autónoma \cite{huval2015empirical} o la predicción (\cite{Dougherty1993, chan2012neural} entre muchos otros.
...
En la misma década, en \cite{Hunt1994} se desarrolló uso de \acrlongsp{ann}\index{red neuronal artificial} aplicadas al entorno del vehículo para identificar el entorno y determinar cuándo y cómo se realiza un cambio de carril\index{lane-change}.
