...
La información que manejan estos agentes dentro de los simuladores suele limitarse a tipologías de vehículo (e.g. utilitarios o vehículos de grandes dimensiones) y magnitudes físicas (tamaño, velocidad máxima). Dependiendo del trabajo, algunos autores añaden más conocimiento a los agentes; por ejemplo, en \cite{hidas2002}, el autor incluye en los agentes, denominados \glspl{dvo}\index{driver-vehicle object} (otra denominación del concepto de \gls{dvu}\index{driver-vehicle unit}), información adicional sobre el tipo de conductor y el nivel de conocimiento de la red de carreteras.
...
Más adelante, en \cite{Das2009}, dentro de su simulador \gls{aasim} añaden los conceptos de \Acrshort{mlc}\index{mandatory lane change} y \Acrshort{dlc}\index{discretional lane change} a los comportamientos de cambos de carril. En \Acrshort{mlc}\index{mandatory lane change} las reglas tienen en cuenta la distancia al siguiente punto característico (e.g. una salida) y el número de cambios de carril necesarios. En \Acrshort{dlc}\index{discretional lane change} deciden si cambiar o no basándose en el nivel de satisfacción del conductor y en el nivel de congestión en los carriles adyacentes\sidenote{
	Este trabajo se apoya en trabajos anteriores que separan las jerarquías en situaciones de \Acrshort{mlc}\index{mandatory lane change} y \Acrshort{dlc}\index{discretional lane change} como \cite{Yang1996, Halati1997, Hidas2002}. Estos trabajos, no obstante, no pertenecen al área de la \gls{ci}.
}.
