...
El objetivo que persigue la simulación de tráfico es hacer cada vez más realistas los modelos generados. Cuando el simulador está basado en \acrlongplsp{mas}, el realismo aumenta cuanto más se parece el comportamiento de los agentes al de los conductores reales.
...
\newthought{con los modelos psicofísicos} se llegó a la conclusión que no todas las situaciones eran iguales, sino que en función del entorno y el momento los umbrales podían variar, y que el \textit{\gls{car-following}\index{car-following}} no era sino un subtipo más de una clase más amplia que se definió como \textit{modelos de aceleración} (figura~\ref{fig:acceleration-model-classes}).
...
\marginnote{
	...
	Esto es sólo un ejemplo, y diferentes autores identifican diferentes situaciones dependiendo del alcance del trabajo como por ejemplo el \textit{\gls{car-following}\index{car-following}} o el \textit{stop-and-go\index{stop-and-go}} \cite{Toledo2003, Liu2013}).
}
...
Desde mediados de los años $1990$ empezó a crecer el interés por aplicar la \acrlongsp{ai} a los \gls{its}. Hay dos razones por las que esto es así: la primera, los éxitos cosechados por la \acrlongsp{ci}, los cuales atrayeron a investigadores de multitud de áreas incluida ésta. La segunda, el rápido desarrollo de la tecnología\sidenote{
...
Algunos autores (\cite{Zhang2011}) se atreven a afirmar incluso que el futuro de las \gls{its} son las técnicas de la \acrlongsp{ci}, y que los resultados que se puedan cosechar de técnicas basadas en el desarrollo convencional de sistemas es marginal comparado con las que se obtendrán con el nuevo enfoque.

Dentro de las \gls{its}, las áreas de aplicación de la \acrlongsp{ci} se centran en los siguientes conceptos: \textbf{reconocimiento de patrones}, \textbf{caracterización de conductores} y \textbf{modelado de conductores}. Es necesario mencionar que aunque se trate de áreas distintas, éstas suelen retroalimentarse, y es difícil encontrar estudios que se centren en una única área sin tocar el resto (figura~\ref{fig:main-applications-of-ci-in-its}). Por ello, aunque nuestro interés pueda centrarse en el modelado de conductores, es necesario conocer el estado de las demás áreas.
...
Las técnicas de \gls{ci} más utilizadas en el modelado de conductores son las \acrlongsp{ann} y la \acrlongsp{fl}. Es comprensible dado que las primeras son una de las técnicas principales en la rama del \acrlongsp{ml}, y la segunda por ser una manera sencilla y cercana a la manera de razonar del ser humano.
...
\newthought{Las \acrlongsp{ann}} fueron el punto de entrada de la \acrlongsp{ci} en la modelización de conductores. Los \acrlongsp{mlp} y sus nuevas técnicas de entrenamiento se habían convertido en la nueva solución universal para todo aquel problema del que se dispusiese de datos.

Las \acrlongsp{ann} se han aplicado mucho sobre el campo de los \acrshort{its} en general, no sólo en modelado de conductores sino en prácticamente todos los aspectos como la clasificación de conductores \cite{DiazAlvarez2014}, los sistemas de conducción autónoma \cite{huval2015empirical} o la predicción (\cite{Dougherty1993, chan2012neural} entre muchos otros.

El primer trabajo de la literatura sobre la aplicación de \acrlongsp{ann} al modelado de conductores es \cite{Fix1990}, donde los autores desarrollan un controlador para imitar el comportamiento de un conductor.

En la misma década, en \cite{Hunt1994} se desarrolló uso de \acrlongsp{ann} aplicadas al entorno del vehículo para identificar el entorno y determinar cuándo y cómo se realiza un cambio de carril\index{lane-change}.
...
El trabajo de \cite{Simonelli2009} también se apoya en datos extraídos de entornos reales. El interés de este estudio radica en que es el primero en realizar una comparativa entre el desempeño de una arquitectura \textit{\idx{fast-forward}} (e.g. \acrlongsp{mlp}) frente a una \idx{recurrente} (e.g. red de Elman). El porqué del uso de redes recurrentes es por su capacidad de reconocer patrones dinámicos, los cuales son de esperar en este tipo de comportamientos.

\newthought{El primer uso de la \acrlongsp{fl}} fue contemporáneo al de las \acrlongplsp{ann}, y también aplicada a un modelo de \textit{\gls{car-following}\index{car-following}}. Después de todo los modelos psicofísicos aparecieron debido a que la percepción del conductor no es absoluta, sino imprecisa.
...
\cite{Kikuchi1992} es el primer trabajo documentado sobre el tema. En él, los autores aplicaron la \acrlongsp{fl} sobre un modelo de aceleración de tipo \textit{\gls{car-following}\index{car-following}}. Utilizaron el modelo \gls{ghr} (Ecuación~\ref{eq:ghr-car-following-model}) como base y determinaron las entradas al modelo como valores de pertenencia a conjuntos difusos. Las entradas del modelo eran las distancia y velocidad relativa entre el vehículo modelado y el delantero y la variación en la aceleración del vehículo delantero. Como salida, el cambio en la tasa de aceleración sobre el vehículo modelado.
...
Los modelos hasta el momento se basaban en conjuntos difusos y reglas definidos \textit{ad-hoc}. En \cite{Chakroborty2003} se introduce el concepto de personalización, ajustando los conjuntos difusos de las variables de entrada y salida a valores extraídos de conductores reales. Este ajuste se realiza mediante la representación del \acrlongsp{fcs} como \acrlongsp{ann} y posterior ajuste mediante entrenamiento de dicha red (\textit{\idx{back-propagation}}). A este trabajo le siguen muchas otras aproximaciónes \textit{\idx{neuro-fuzzy}} como \cite{mbede2004robust, Zheng2005}
...
\newthought{Las \acrlongplsp{ann} y la \acrlongsp{fl} no son} las únicas técnicas usadas para determinar comportamientos en conductores.

Por ejemplo, en \cite{maye2011bayesian} se presenta un modelo no supervisado, online y basado en redes bayesianas donde se infiere el comportamiento del conductor haciendo uso de una \gls{imu} y una cámara. De la \gls{imu} se extraen datos que se separan en fragmentos para luego relacionarlos con las imágenes obtenidas de la cámara. Otro trabajo similar pueden ser \cite{van2013driver} el cual se apoya también en la segmentación de los datos extraídos de una \gls{imu} pero con técnicas distintas (concretamente \textit{clustering} basado en \Acrfullpl{svm} y en $k$-medias) y sin cámara.
...
\newthought{Estas técnicas no sólo se usan para} modelar comportamientos complejos o modelos enteros. Algunos trabajos se ocupan de características o aspectos de un modelo concreto. Por ejemplo, los trabajos \cite{Hatipkarasulu2002, zheng2013} se ocupan exclusivamente del cálculo de tiempo de respuesta del conductor en modelos de \textit{\gls{car-following}\index{car-following}}, el primero con \acrlongplsp{fcs} y el segundo con \acrlongplsp{ann}.