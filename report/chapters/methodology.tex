...
Los modelos de conducción propuestos se basarán en datos extraídos de entornos reales. Tras su generación, se integrarán posteriormente en el entorno de simulación \gls{sumo}\index{SUMO} para comprobar su desempeño. Es importante por tanto reparar en los siguientes detalles:
...
El entorno de simulación con el que trabajamos, \gls{sumo}\index{SUMO} impone ciertas limitaciones, siendo una de las más decisivas el funcionamiento \enquote{por carril}. Esto quiere decir aunque el simulador sea continuo en el espacio, su recorrido longitudinal funciona en una única dimensión, siendo el cambio de carril\index{lane-change} el salto de un carril a otro.
...
Se ha hecho uso de un vehículo instrumentado con sensores para capturar la mayor cantidad posible de la información indicada. El vehículo en cuestión es un Mitsubishi iMiEV (Figura~\ref{fig:instrumented-imiev}) y los dispositivos un \acrfull{lidar}\index{LiDAR}, un \gls{gps}, el Bus \Acrfull{can} y una cámara.
...
Para cada uno de los submodelos se han probado dos aproximaciones diferentes para comparar sus desempeños en las tareas por separado. Concretamente se han utilizado \acp{fcs}\index{sistema de control difuso} y \acp{mlp}\index{perceptrón multicapa} para modelar el comportamiento longitudinal y \acp{mlp}\index{perceptrón multicapa} y \acp{cnn}\index{red de convolución} para modelar el comportamiento en cambio de carril\index{lane-change}.

Los modelos serán entrenados con un conjuntos por tipo de modelo (longitudinal y cambio de carril\index{lane-change}) asociado al total de sujetos. Tras ello, y una vez decididas las arquitecturas de los modelos que se usarán para cada tipo, se entrenarán para cada uno de los sujetos por separado.